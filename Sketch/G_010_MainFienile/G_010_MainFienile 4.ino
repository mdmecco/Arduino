...
#include "A:\ArduinoEnv\libraries\A_Mecco\pulsanti.h"
...
unsigned long TApri = 54000;
unsigned long TChiudi= 82800;
unsigned long TMuovi = 20000;
...
//-----------  Codice dentro rete  ---------    
    ...
    if ((DaySec() > TApri) && APCheck){
      APCheck=false;
      ETH484Sw(0);
      MilliApriP=millis()+TMuovi; 
    }

    if ((DaySec() > TChiudi) && CPCheck && (millis()>MilliApriP)){
      CPCheck=false;
      ETH484Sw(1); 
    }
