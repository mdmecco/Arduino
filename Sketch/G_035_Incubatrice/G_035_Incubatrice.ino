...
#define WEBTITPAGE "Incubatrice"
#define PRGVER "2021-03-05 V2.0"
...
static unsigned char MBUS01Input[] = {0x01, 0x03, 0x00, 0x81, 0x01, 0x08, 0x15, 0xB4};
...
// ************************   Codice fuori rete ****************************************
  ...
  if (Tem > (TCova + dTCova)){
    ...
    digitalWrite(RISC, false);
  }
  if (Tem < (TCova - dTCova)){
    //Accendo il riscaldatore
    digitalWrite(RISC, true);
    
  }


  if (Hum > (HCova + dHCova)){
    // Spengo l'umidificatore
    digitalWrite(ARIA, false);
  }
  if (Hum < (HCova - dHCova)){
    //Accendo l'umidificatore
    digitalWrite(ARIA, true);
    
  }

  if ((millis() > TimeRoB) && (millis() > TimeRaB)){
      //------ Attiva Rotazione
      digitalWrite(ROT, false);
      TimeRaB=millis() + TimeRa;
      TimeRoB=millis() + TimeRo;
  }
  if ((millis() < TimeRoB) && (millis() > TimeRaB)){
    //----- ferma la rotazione
    digitalWrite(ROT, true);
